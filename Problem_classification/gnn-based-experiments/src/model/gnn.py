...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_vocab, max_seq_len, node_encoder, num_layer = 5, emb_dim = 300, 
        ...
        self.attentation = GlobalAttention(gate_nn = torch.nn.Sequential(torch.nn.Linear(emb_dim, 2*emb_dim), torch.nn.BatchNorm1d(2*emb_dim), torch.nn.ReLU(), torch.nn.Linear(2*emb_dim, 1)))
    ...
    def forward(self, batched_data):
        ...
        h_attention = self.attentation(h_node, batched_data.batch)
        
        ##Here, you can use hybrid pooling methods. For example h_enhance = torch.add(h_graph, h_attention)

        if self.num_class > 0:
            return self.graph_pred_linear(h_graph), self.graph_pred_linear(h_attentation)

        pred_list_graph = []
        pred_list_attentation = []
        for i in range(self.max_seq_len):
            pred_list_graph.append(self.graph_pred_linear_list[i](h_graph))
            pred_list_attentation.append(self.graph_pred_linear_list[i](h_attentation))
            

        return pred_list_graph, pred_list_attentation         #Hardmard Matrix Multiplication
